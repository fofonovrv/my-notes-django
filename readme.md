...
Posts.objects.filter(cat__name__contains='er')  # выбрать все посты, в названии категорий которых содержится 'er'
```
...
```
### Получение предыдущей и следующей записи
Используем метод get_previous_by_ и get_next_by_ вместе с именем необходимого поля. В нашем случае, time_create:
```
p = Posts.objects.get(pk=4)  # запись с id=4
p.get_previous_by_time_create()  # предыдущая запись по времени создания, относительно записи "p"
p.get_next_by_time_create()  # следующая запись

```
### Проверка существования записи. Число записей
Проверим, существуют ли посты (в таблице posts) с категорией "Docker" (таблица Category):
```
c = Category.object.get(name='Docker') # присваеваем переменной объект записи с именем категории Docker
c.posts_set.exist()  # ищем записи в таблице Posts по связанному внешнему ключу категории
```
Если записи существуют, получаем True, в противном случае - False
Аналогично можно получить количество постов с категорией Docker:
```
c.posts_set.count() 
```
### 
